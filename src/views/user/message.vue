...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 通知未读数量
      unReadNum1() {
        return this.allData.filter((d) => d.state === 0 && d.type === 'notice')
          .length;
      },
      // 私信未读数量
      unReadNum2() {
        return this.allData.filter((d) => d.state === 0 && d.type === 'message')
          .length;
      },
      // 代办未读数量
      unReadNum3() {
        return this.allData.filter((d) => d.state === 0 && d.type === 'todo')
          .length;
      }
    },
    created() {
    ...
    methods: {
      ...
      remove(row) {
        this.allData.splice(
          this.allData.findIndex((t) => t.id === row.id),
          1
        );
        ...
        this.$confirm('确定要删除选中的消息吗?', '提示', {
          ...
          .then(() => {
            this.allData = this.allData.filter((d) => {
              return !this.selection.some((t) => t.id === d.id);
            });
            this.changeType();
            this.$message.success('删除成功');
          })
    ...
    watch: {
      $route: {
        handler(route) {
          this.active = route?.query?.type || 'all';
          this.changeType();
        },
        immediate: true
      }
