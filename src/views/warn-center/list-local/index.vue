    ...
    <el-card shadow="never" body-style="padding-bottom: 0;">
      <el-row :gutter="15">
        ...
        <el-col :lg="18" :md="18" :xl="18" :sm="18" style="margin-bottom: 15px;height: 100%;">
          <!-- 数据字典项列表 -->
          <dict-data :if="loadallDevices" :loadall-devices="loadallDevices" :Cam_showcount="Cam_showcount" :current="current" :audioAlarms="audioAlarms"  @selectvideoindex="selectvideoindex" style="height: 100%;" />
        </el-col>
        <el-col :lg="6" :md="6" :xl="6" :sm="6" style="margin-bottom: 15px">
          ...
          <template slot="toolbar">
            <el-select v-model="Cam_showcount" placeholder="请选择" style="width: 10vw;">
              <el-option key="1" label="同时打开一个视频" value="1"></el-option>
              <el-option key="4" label="同时打开四个视频" value="4"></el-option>
            </el-select>
            ...
            <a download="rtsp2webrtc_amd64.zip" href="/rtsp2webrtc_amd64.zip" style="color: red;z-index: 9999;position: fixed;right: 35px;bottom: 16px;">下载插件</a>
          </template>
          </ele-pro-table>
        </el-col>
      </el-row>
...
<script>
  ...
  import { loadallDevices } from '@/api/config/channel_config';
      ...
      datasource() {
        loadallDevices().then((res)=>{
          ...
          for(let i = 0; i< res.list.length; i++){
						this.loadallDevices[i].src = window.location.protocol+"//"+window.location.hostname+":"+(res.list[i].Cam_http);
            // this.loadallDevices[i].src = "http://192.168.49.122:"+(res.list[i].Cam_http);
          }
        })
