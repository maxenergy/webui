      ...
      async datasource() {
        await loadallDevices().then((res)=>{
          ...
          for(let i = 0; i< res.list.length; i++){
						this.loadallDevices[i].src = window.location.protocol+"//"+window.location.hostname+":"+(res.list[i].Cam_http)+"?time="+new Date().getTime();
            // this.loadallDevices[i].src = "http://192.168.2.168:"+(res.list[i].Cam_http)+"?time="+new Date().getTime();
            aigetconfig({
                device: res.list[i]
              })
              .then((res1) => {
